...
import { Plus, Trash2, Save, FolderOpen, X, TrendingUp, Sun, Moon, Printer } from "lucide-react";
...
const defaultStages = [
  ...
  { id: "s3", name: "Opportunity \u2192 Close Won", rate: 25 },
];
  ...
  return (
    <div className="app-root min-h-screen font-sans transition-colors duration-200" data-theme={theme}>
      <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={addStage} className="no-print link-accent flex items-center gap-1 text-xs">
                  <Plus size={14} /> Stage
                </button>
              </div>
                ...
                {stages.map((s, i) => (
                  <div key={s.id} className="flex items-center gap-2 bg-panel border border-base rounded-md px-3 py-2">
                    ...
                    <button
                      ...
                      className="no-print text-faint hover-danger disabled:opacity-30"
                    >
                      <X size={14} />
                    </button>
                  </div>
            ...
            {/* Save scenario */}
            <section className="no-print space-y-2 pt-2 border-t border-base">
        ...
        {/* Sales funnel */}
        <div className="mt-8 bg-panel border border-base rounded-lg p-5">
          <p className="text-xs font-mono uppercase tracking-widest text-label mb-1">Sales funnel</p>
          <p className="text-xs text-faint mb-4">
            Read it left to right: the number under each stage is how many you need there, and each %
            is the share that makes it through to the next one.
          </p>
          {(() => {
            let volume = dealsNeeded;
            const rows = [{ name: "Close Won", volume }];
            ...
            const maxThickness = 130;
            const thickness = (v) =>
              maxVolume === minVolume ? maxThickness : minThickness + ((v - minVolume) / (maxVolume - minVolume)) * (maxThickness - minThickness);

            const margin = 20;
            const svgWidth = 920;
            const colWidth = (svgWidth - margin * 2) / Math.max(1, n - 1);
            const labelY = 14;
            const numberY = 42;
            const pctY = 62;
            const flowTop = 74;
            const svgHeight = flowTop + maxThickness + 16;

            const xAt = (i) => margin + i * colWidth;
            const bottomAt = (i) => flowTop + thickness(rows[i].volume);
            const xLast = xAt(n - 1);

            let curveCommands = "";
            for (let i = n - 2; i >= 0; i--) {
              const midX = (xAt(i) + xAt(i + 1)) / 2;
              curveCommands += ` C ${midX},${bottomAt(i + 1)} ${midX},${bottomAt(i)} ${xAt(i)},${bottomAt(i)}`;
            }
            const fullPath = `M ${xAt(0)},${flowTop} L ${xLast},${flowTop} L ${xLast},${bottomAt(n - 1)} ${curveCommands} Z`;

            return (
              <>
                <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} width="100%" style={{ display: "block" }}>
                  <defs>
                    <linearGradient id="funnelFlow" gradientUnits="userSpaceOnUse" x1={xAt(0)} x2={xLast} y1="0" y2="0">
                      <stop offset="0%" stopColor="var(--accent)" stopOpacity="0.12" />
                      <stop offset="50%" stopColor="var(--accent)" stopOpacity="0.45" />
                      <stop offset="100%" stopColor="var(--accent)" stopOpacity="1" />
                    </linearGradient>
                  </defs>
                  <path d={fullPath} fill="url(#funnelFlow)" />
                  {Array.from({ length: bandCount }).map((_, i) => {
                    const midX = (xAt(i) + xAt(i + 1)) / 2;
                    const pct = Math.round(stages[i].rate);
                    return (
                      <text key={i} x={midX} y={pctY} fontSize="12" fontFamily="ui-monospace, monospace" fill="var(--text-muted)" textAnchor="middle">
                        {`${pct}%`}
                      </text>
                    );
                  })}
                  {rows.map((r, i) => {
                    ...
                    return (
                      <g key={i}>
                        <text x={xAt(i)} y={labelY} fontSize="10" letterSpacing="0.05em" fill="var(--text-label)" textAnchor={anchor}>
                          {r.name.toUpperCase()}
                        </text>
                        <text x={xAt(i)} y={numberY} fontSize="22" fontWeight="600" fontFamily="ui-monospace, monospace" fill="var(--text-primary)" textAnchor={anchor}>
                          {fmtInt(r.volume)}
                        </text>
                      </g>
                    );
                ...
                <p className="text-xs text-faint mt-2 pt-3 border-t border-base">
                  Bottom line: you need <span className="text-body font-mono">{fmtInt(rows[0].volume)}</span> leads to land{" "}
                  <span className="text-body font-mono">{fmtInt(dealsNeeded)}</span> closed deals.
                </p>
              </>
            );
          })()}
